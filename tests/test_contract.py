...
TEXT1 = "The fair use of a copyrighted work for criticism comment news reporting teaching scholarship or research is not an infringement of copyright."
...
def mocks(vm, states=("SUPPORTED", "SUPPORTED")):
    ...
    vm.mock_llm("CITEGUARD", json.dumps({"findings":[{"index":0,"state":states[0],"source_indexes":[0],"pinpoint_quote":"purpose and character of the use"},{"index":1,"state":states[1],"source_indexes":[1],"pinpoint_quote":"effect of the use upon the potential market"}]}))
...
def test_malformed_attribution_is_rejected(direct_vm, direct_deploy):
    ...
    direct_vm.mock_llm("CITEGUARD", json.dumps({"findings":[{"index":0,"state":"SUPPORTED","source_indexes":[99],"pinpoint_quote":"invented attribution"},{"index":1,"state":"SUPPORTED","source_indexes":[1],"pinpoint_quote":"market effect"}]}))
...
def test_appeal_is_creator_only_new_source_and_deadline_protected(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    with direct_vm.expect_revert("requires one new authority"): c.appeal_audit(mid)
